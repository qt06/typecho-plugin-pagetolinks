 ...
 * @author qining
 * @modified by 杨永全 http://www.qt06.com/
 * @version 1.0.1
...
class PageToLinks implements Typecho_Plugin_Interface
{
     ...
     * @return void
     */
    public static function output($slug = 'links', $tag = 'h3', $listTag = 'ul')
    {
        /** 获取数据库支持 */
        $db = Typecho_Db::get();
        ...
        if (isset($contents->text)) {
            $contents->isMarkdown = (0 === strpos($contents->text, '<!--markdown-->'));
            if ($contents->isMarkdown) {
                $contents->text = substr($contents->text, 15);
            }
        }
        if($contents->isMarkdown) {
            $contents->text = Markdown::convert($contents->text);
        }
